test_that("Scoring of variant in family multiple families proceeds as expected", {
  ...
  tsv.name1<-system.file('extdata/4107_gcn1.mat', package = 'seqbio.variant.scoring')
  ...
  tsv.name2<-system.file('extdata/4974_gcn1.mat', package = 'seqbio.variant.scoring')
  ...
  tsv.name3<-system.file('extdata/8363_gcn1.mat', package = 'seqbio.variant.scoring')
  gcn1_8363.df <- read.indiv(tsv.name3)

  gcn1_8383.df.status <-  score.variant.status(gcn1_8383.df)
  gcn1_8383.score <- score.fam(gcn1_8383.mat, gcn1_8383.df.status, affected.weight=1, unaffected.weight=0.5, return.sums = TRUE)


  #TODO - make this function
  sum.fam.scores(c(gcn1_4107.score, gcn1_4974.score, gcn1_8383.score))
